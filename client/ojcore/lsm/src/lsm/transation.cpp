...
void TranManager::read_tranc_id_file() {
  // 文件为空或损坏（如进程被强杀后遗留的 0 字节文件）时使用默认值，
  // 避免 read_uint64 抛 out_of_range 导致启动崩溃
  const size_t file_size = tranc_id_file_.size();
  ...
  try {
    ...
    for (uint64_t i = 0; i < size; i++) {
      if (offset + sizeof(uint64_t) > file_size) break;
...
std::map<uint64_t, std::vector<Record>> TranManager::check_recover() {
  ...
  // 保险：flushedTrancIds_ 为空时兜底为 {0}，避免 begin() 解引用 UB
